...
export var createChannel = async ({title, desc, myValue,paidperson2, finishtime1, finishtime2}) => {
  var newChannelRef = DB.collection('channels').doc();
  var id = newChannelRef.id;
  var newChannel = {
    id,
    title,
    description: desc,
...
export const createPaid2 = async ({channelId, paidperson2}) => {
  return await DB.collection('channels')
    .doc(channelId)
    .add(paidperson2)
}